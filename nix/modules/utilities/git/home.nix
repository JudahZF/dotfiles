...
{
  home.activation.configureDotfilesGitHooks = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    dotfilesDir="${config.home.homeDirectory}/dotfiles"
    if [ -d "$dotfilesDir/.git" ]; then
      ${lib.getExe pkgs.git} -C "$dotfilesDir" config core.hooksPath "$dotfilesDir/.githooks"
    fi
  '';
